/*******************************************************************************
"Programa suma con query para medir tiempo t(n)
*******************************************************************************/
...
int main(){
	...
	//	cout<<"Tiempo: "<< GetCounter()<<"ms."<<endl;
	 ...
	 cout<<"si quieres repetir R ->";
	 cin >> retry;
    }
	while('q'==retry || 'Q'!=retry || 'r'==retry || 'R'==retry);
...
void sumaMatrices(int **M1,int **M2,int ** MS,int n,int m){
...
void mostrarMatrizS(int **MS,int n,int m){
	cout<<"Imprimiendo Matriz Suma"<<endl<<endl;
	for(int i=0;i<n;i++){
		for(int j=0;j<m;j++)
			cout<<"|"<<MS[i][j]<<"|";
	cout<<endl;
